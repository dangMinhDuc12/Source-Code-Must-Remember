//Call DOM element and get id of DashBoard and TempPage
...
//Logic show Content for content page
...
email.onclick = function () {
  email.setAttribute(
    "href",
    `mailto:nmhung1234@hotmail.com?subject=${nameMailUser.value}&body=${contentEmail.value}`
  );
...
buttonComment.onclick = function () {
  ...
  postData("http://localhost:3000/commentUser", formData).then(function (ss) {
    console.log(ss);
  });
  showComment();
};
...
allComment.onclick = function (e) {
  if (e.target.classList.contains("del")) {
    ...
    deleteData(`http://localhost:3000/commentUser/${e.target.id}`).then(
      function (ss) {
        console.log(ss);
      }
    );
